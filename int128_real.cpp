...
public:
    ...
    Int128 operator*(const Int128& other) const {
        ...
        // ƒобавл€ем вклад от a_hi и b_hi
        hi += a_hi * b_lo + b_hi * a_lo;
        // —тарша€ часть 128-битного произведени€ (в дополнительном коде дл€ беззнакового)
        // Ќо нам нужно 128 бит, старша€ часть hi уже содержит 64 бита, остальное отбросим (переполнение)

        // —обираем результат
        Int128 res(lo, static_cast<int64_t>(hi));
        return sign ? -res : res;

        // по€сню: разбиваем на еще болнее мелкие кусочки,
        // чтобы не было переполнени€ в промежуточных вычислени€х
    }
...
private:
    uint64_t lo_;
    int64_t hi_;  // знакова€ старша€ часть (дополнительный код)
    ...
    static void div_mod(const Int128& dividend, const Int128& divisor,
        ...
        // реализаци€ алгоритма двоичного делени€ (тоже украл)
        quotient = Int128(0);
        remainder = Int128(0);

        for (int i = 127; i >= 0; --i) {
            remainder = remainder << 1;
            // берЄм i-й бит делимого
            uint64_t bit = (dividend.hi_ >= 0 ?
                (static_cast<uint64_t>(dividend.hi_) >> (i - 64)) :
                // дл€ отрицательных чисел не попадаем, т.к. abs()
                0);
            if (i >= 64) {
                int shift = i - 64;
...
public:
    ...
    Int128 eval(const std::map<std::string, Int128>& vars) const override {
        ...
        // ≈сли переменна€ не найдена, возвращаем 0
